...
int	handle_redouts(t_tkn *redouts)
{
	int	fd;
...
int	handle_redins(t_tkn *redins)
{
	int	fd;
...
int	handle_redirs(t_exec *pipeline)
{
	...
	if (pipeline->redins)
	{
		fd_in = handle_redins(pipeline->redins);
		if (fd_in == -1)
			return (0);
	...
	if (pipeline->redouts)
	{
		fd_out = handle_redouts(pipeline->redouts);
		if (fd_out == -1)
			return (0);
